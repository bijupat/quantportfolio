...
def get_quality_scores_bulk(symbols: list[Symbol], as_of: date) -> QuerySet[QualityScore]:
    ...
    if missing_symbols:
        ...
        for symbol in missing_symbols:
            try:
                # Run Original Scripts — pass the DB-backed is_financial flag so
                # Altman's exclusion decision uses it instead of ticker-string matching.
                p_result = calculate_piotroski_score(symbol.ticker)
                a_result = calculate_altman_z_score(
                    symbol.ticker,
                    is_financial_hint=symbol.is_financial,
                )
